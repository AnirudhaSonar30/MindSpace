// MindSpace — Atmosphere Layer
// Full-viewport canvas sitting above the Three.js sky.
// Renders scene-specific particles, fog, glow, and effects.
// Uses mix-blend-mode: screen so particles glow additively on the dark sky.

import { useEffect, useRef, useState } from 'react'
import { sceneEngine, Scene, SCENES } from './scenes'

declare global {
  interface Window {
    __mindspaceScene?:     Scene
    __mindspaceScenePrev?: Scene
    __mindspaceSceneT?:    number
  }
}

const REDUCED_ATM = window.matchMedia('(prefers-reduced-motion: reduce)').matches

const easeInOut = (t: number) => t < 0.5 ? 2 * t * t : -1 + (4 - 2 * t) * t

/* ═══════════════════════════════════════════════════════════════
   Particle interfaces
═══════════════════════════════════════════════════════════════ */
interface RainDrop  { x: number; y: number; speed: number; len: number; op: number; w: number }
interface MistPuff  { x: number; y: number; rx: number; ry: number; op: number; vx: number; vy: number; life: number; maxLife: number }
interface PollenDot { x: number; y: number; r: number; op: number; vx: number; vy: number; phase: number }
interface DriftDot  { x: number; y: number; r: number; op: number; vx: number; vy: number; phase: number; waveAmp: number }
interface Ember     { x: number; y: number; r: number; op: number; vx: number; vy: number; phase: number }
interface Star      { x: number; y: number; r: number; op: number; twinklePhase: number; twinkleSpeed: number; px: number; py: number }
interface Streak    { x: number; y: number; len: number; speed: number; op: number; w: number }
interface CityLight { x: number; y: number; r: number; op: number; maxOp: number; col: [number,number,number]; life: number; maxLife: number; state: 'in'|'hold'|'out' }
interface Bird      { x: number; y: number; hw: number; op: number; vx: number; vy: number; phase: number; flapRate: number; flapAmp: number }
interface ForestTree  { cx: number; baseY: number; h: number; hw: number; phase: number; phase2: number }
interface ForestLayer { trees: ForestTree[]; color: string; op: number }
interface BoltPath    { segs: [number,number][]; branch: [number,number][] }

/* ═══════════════════════════════════════════════════════════════
   Particle pool initializers
═══════════════════════════════════════════════════════════════ */
function makeRainDrop(W: number, H: number, init = false): RainDrop {
  return {
    x: Math.random() * (W + 100) - 50,
    y: init ? Math.random() * H : -(10 + Math.random() * 60),
    speed: 280 + Math.random() * 220,
    len: 8 + Math.random() * 18,
    op: 0.03 + Math.random() * 0.09,
    w: 0.25 + Math.random() * 0.4,
  }
}
function makeMistPuff(W: number, H: number): MistPuff {
  return {
    x: Math.random() * W, y: Math.random() * H,
    rx: 80 + Math.random() * 200, ry: 30 + Math.random() * 80,
    op: 0.025 + Math.random() * 0.045,
    vx: (Math.random() - 0.5) * 8, vy: (Math.random() - 0.5) * 4,
    life: 0, maxLife: 6 + Math.random() * 8,
  }
}
function makePollenDot(W: number, H: number): PollenDot {
  return {
    x: Math.random() * W, y: Math.random() * H,
    r: 0.8 + Math.random() * 1.8,
    op: 0.18 + Math.random() * 0.45,
    vx: (Math.random() - 0.5) * 10,
    vy: -(2 + Math.random() * 8),
    phase: Math.random() * Math.PI * 2,
  }
}
function makeDriftDot(W: number, H: number): DriftDot {
  return {
    x: Math.random() * W, y: Math.random() * H,
    r: 0.6 + Math.random() * 2.2,
    op: 0.10 + Math.random() * 0.35,
    vx: (Math.random() - 0.5) * 6,
    vy: (Math.random() - 0.5) * 3,
    phase: Math.random() * Math.PI * 2,
    waveAmp: 8 + Math.random() * 20,
  }
}
function makeEmber(W: number, H: number, init = false): Ember {
  return {
    x: W * 0.3 + Math.random() * W * 0.4,
    y: init ? Math.random() * H : H + 5,
    r: 0.8 + Math.random() * 2.4,
    op: 0.4 + Math.random() * 0.5,
    vx: (Math.random() - 0.5) * 22,
    vy: -(30 + Math.random() * 55),
    phase: Math.random() * Math.PI * 2,
  }
}
function makeStar(W: number, H: number): Star {
  return {
    x: Math.random() * W, y: Math.random() * H,
    r: 0.25 + Math.random() * 1.1,
    op: 0.15 + Math.random() * 0.70,
    twinklePhase: Math.random() * Math.PI * 2,
    twinkleSpeed: 0.3 + Math.random() * 1.2,
    px: 0, py: 0,
  }
}
function makeStreak(W: number, H: number, init = false): Streak {
  return {
    x: init ? Math.random() * W * 1.3 - W * 0.15 : -(60 + Math.random() * 160),
    y: Math.random() * H,
    len: 60 + Math.random() * 180,
    speed: 200 + Math.random() * 280,
    op: 0.04 + Math.random() * 0.10,
    w: 0.3 + Math.random() * 1.0,
  }
}
const CITY_LIGHT_COLORS: [number,number,number][] = [
  [255, 220, 140],
  [180, 210, 255],
  [255, 240, 180],
  [200, 225, 255],
  [255, 200, 120],
]
function makeCityLight(W: number, H: number): CityLight {
  const side = Math.random() < 0.5 ? 'left' : 'right'
  const col  = CITY_LIGHT_COLORS[Math.floor(Math.random() * CITY_LIGHT_COLORS.length)]
  return {
    x: side === 'left' ? Math.random() * W * 0.25 : W * 0.75 + Math.random() * W * 0.25,
    y: H * 0.2 + Math.random() * H * 0.65,
    r: 1.2 + Math.random() * 2.5,
    op: 0, maxOp: 0.18 + Math.random() * 0.22,
    col,
    life: 0, maxLife: 0.8 + Math.random() * 2.0,
    state: 'in',
  }
}
function makeBird(W: number, H: number, init = false): Bird {
  const hw = 22 + Math.random() * 16
  return {
    x:        Math.random() * W,
    y:        init ? H * 0.08 + Math.random() * H * 0.48 : -30,
    hw,
    op:       0.65 + Math.random() * 0.22,
    vx:       (Math.random() < 0.5 ? -1 : 1) * (14 + Math.random() * 18),
    vy:       (Math.random() - 0.5) * 3,
    phase:    Math.random() * Math.PI * 2,
    flapRate: 0.7 + Math.random() * 1.4,
    flapAmp:  hw * (0.22 + Math.random() * 0.14),
  }
}

/* ═══════════════════════════════════════════════════════════════
   Main Atmosphere Canvas
═══════════════════════════════════════════════════════════════ */
export function AtmosphereCanvas() {
  const canvasRef = useRef<HTMLCanvasElement | null>(null)

  useEffect(() => {
    if (REDUCED_ATM) return
    const canvas = canvasRef.current
    if (!canvas) return
    const ctx = canvas.getContext('2d')!
    let W = window.innerWidth, H = window.innerHeight
    let animId: number

    const resize = () => {
      const d = Math.min(window.devicePixelRatio || 1, 2)
      W = window.innerWidth; H = window.innerHeight
      canvas.width  = W * d; canvas.height = H * d
      canvas.style.width = W + 'px'; canvas.style.height = H + 'px'
      ctx.setTransform(d, 0, 0, d, 0, 0)
      initPools()
    }

    let rain: RainDrop[] = [], mist: MistPuff[] = [], pollen: PollenDot[] = []
    let drift: DriftDot[] = [], embers: Ember[] = [], stars: Star[] = []
    let streaks: Streak[] = [], cityLights: CityLight[] = [], birds: Bird[] = []
    let forestTrees: ForestLayer[] = []

    function buildForestTrees() {
      const rng = (a: number, b: number) => a + Math.random() * (b - a)
      forestTrees = []
      type LayerDef = [number, number, number, string, number]
      const layerDefs: LayerDef[] = [
        [0.82, 28, 0.082, 'rgba(1,7,2,1)',  1.00],
        [0.74, 16, 0.120, 'rgba(0,3,1,1)',  1.00],
      ]
      for (const [baseYF, n, hF, color, op] of layerDefs) {
        const spacing = W / n
        const trees: ForestTree[] = []
        for (let i = 0; i < n; i++) {
          trees.push({
            cx:    spacing * (i + 0.5) + rng(-spacing * 0.18, spacing * 0.18),
            baseY: H * baseYF + rng(-H * 0.012, H * 0.012),
            h:     H * hF * rng(0.60, 1.40),
            hw:    spacing * rng(0.50, 0.72),
            phase:  rng(0, Math.PI * 2),
            phase2: rng(0, Math.PI * 2),
          })
        }
        trees.sort((a, b) => a.cx - b.cx)
        forestTrees.push({ trees, color, op })
      }
    }

    function initPools() {
      rain       = Array.from({ length: 55  }, () => makeRainDrop(W, H, true))
      mist       = Array.from({ length: 18  }, () => makeMistPuff(W, H))
      pollen     = Array.from({ length: 55  }, () => makePollenDot(W, H))
      drift      = Array.from({ length: 45  }, () => makeDriftDot(W, H))
      embers     = Array.from({ length: 35  }, () => makeEmber(W, H, true))
      stars      = Array.from({ length: 190 }, () => makeStar(W, H))
      streaks    = Array.from({ length: 28  }, () => makeStreak(W, H, true))
      cityLights = Array.from({ length: 12  }, () => makeCityLight(W, H))
      birds      = Array.from({ length: 9   }, () => makeBird(W, H, true))
      buildForestTrees()
    }

    let lastTs   = 0
    let flashT   = 0
    let boltPath: BoltPath | null = null

    function generateBolt(): BoltPath {
      const startX = W * (0.15 + Math.random() * 0.70)
      const steps  = 10 + Math.floor(Math.random() * 6)
      const segs: [number,number][] = [[startX, 0]]
      let x = startX
      for (let i = 1; i <= steps; i++) {
        x += (Math.random() - 0.5) * W * 0.18
        x = Math.max(W * 0.05, Math.min(W * 0.95, x))
        segs.push([x, H * 0.82 * (i / steps)])
      }
      const bi = Math.floor(steps * 0.3 + Math.random() * steps * 0.35)
      const branch: [number,number][] = [segs[bi]]
      let bx = segs[bi][0], by = segs[bi][1]
      const bsteps = 3 + Math.floor(Math.random() * 3)
      for (let i = 1; i < bsteps; i++) {
        bx += (Math.random() - 0.45) * W * 0.14
        by += H * 0.28 / bsteps
        branch.push([bx, by])
      }
      return { segs, branch }
    }

    function drawSceneGlow(scene: Scene, alpha: number) {
      const [r, g, b] = scene.glow
      const grad = ctx.createRadialGradient(W * 0.5, H * 0.55, 0, W * 0.5, H * 0.55, Math.max(W, H) * 0.75)
      grad.addColorStop(0, `rgba(${r},${g},${b},${(0.08 + scene.fogDensity * 0.08) * alpha})`)
      grad.addColorStop(1, 'rgba(0,0,0,0)')
      ctx.fillStyle = grad
      ctx.fillRect(0, 0, W, H)
    }

    function drawFog(scene: Scene, t: number, alpha: number) {
      const [r, g, b] = scene.fog
      const d = scene.fogDensity
      for (let i = 0; i < 4; i++) {
        const ox  = Math.sin(t * 0.07 + i * 1.3) * W * 0.22
        const oy  = Math.cos(t * 0.05 + i * 0.9) * H * 0.18
        const cx2 = W * (0.15 + i * 0.22) + ox
        const cy2 = H * (0.3  + (i % 2) * 0.4) + oy
        const rx  = W * (0.30 + i * 0.08)
        const ry  = H * (0.22 + i * 0.04)
        const op  = d * (0.10 + i * 0.025) * alpha
        const g2  = ctx.createRadialGradient(cx2, cy2, 0, cx2, cy2, rx)
        g2.addColorStop(0, `rgba(${r},${g},${b},${op})`)
        g2.addColorStop(1, 'rgba(0,0,0,0)')
        ctx.fillStyle = g2
        ctx.beginPath()
        ctx.ellipse(cx2, cy2, rx, ry, 0, 0, Math.PI * 2)
        ctx.fill()
      }
    }

    function drawColorGrade(scene: Scene, alpha: number) {
      const [r, g, b] = scene.tint
      ctx.fillStyle = `rgba(${r},${g},${b},${scene.tintOpacity * alpha})`
      ctx.fillRect(0, 0, W, H)
    }

    function drawRain(dt: number, scene: Scene, alpha: number) {
      const SIN = Math.sin(76 * Math.PI / 180)
      const COS = Math.cos(76 * Math.PI / 180)
      const ms  = scene.motionSpeed
      for (let i = 0; i < rain.length; i++) {
        const d = rain[i]
        d.x += d.speed * ms * dt * COS
        d.y += d.speed * ms * dt * SIN
        if (d.y > H + 20) { rain[i] = makeRainDrop(W, H, false); continue }
        ctx.save()
        ctx.globalAlpha = d.op * alpha
        ctx.strokeStyle = 'rgba(140,185,255,1)'
        ctx.lineWidth = d.w
        ctx.lineCap = 'round'
        ctx.beginPath()
        ctx.moveTo(d.x - d.len * COS, d.y - d.len * SIN)
        ctx.lineTo(d.x, d.y)
        ctx.stroke()
        ctx.restore()
      }
    }

    function drawMist(dt: number, _t: number, scene: Scene, alpha: number) {
      const ms = scene.motionSpeed
      for (let i = 0; i < mist.length; i++) {
        const p = mist[i]
        p.x += p.vx * ms * dt
        p.y += p.vy * ms * dt
        p.life += dt
        if (p.life > p.maxLife || p.x < -300 || p.x > W + 300) { mist[i] = makeMistPuff(W, H); continue }
        const fade = Math.sin((p.life / p.maxLife) * Math.PI)
        const g2   = ctx.createRadialGradient(p.x, p.y, 0, p.x, p.y, p.rx)
        g2.addColorStop(0, `rgba(160,170,210,${p.op * fade * alpha})`)
        g2.addColorStop(1, 'rgba(0,0,0,0)')
        ctx.fillStyle = g2
        ctx.beginPath()
        ctx.ellipse(p.x, p.y, p.rx, p.ry, 0, 0, Math.PI * 2)
        ctx.fill()
      }
    }

    function drawPollen(dt: number, t: number, scene: Scene, alpha: number) {
      const ms = scene.motionSpeed
      for (let i = 0; i < pollen.length; i++) {
        const p = pollen[i]
        p.x += (p.vx + Math.sin(t * 0.4 + p.phase) * 12) * ms * dt
        p.y += p.vy * ms * dt
        if (p.y < -20) { pollen[i] = makePollenDot(W, H); pollen[i].y = H + 5; continue }
        if (p.x < -20 || p.x > W + 20) p.x = p.x < 0 ? W + 10 : -10
        const twinkle = 0.65 + 0.35 * Math.sin(t * 1.1 + p.phase)
        ctx.save()
        ctx.globalAlpha = p.op * twinkle * alpha
        ctx.fillStyle = 'rgba(220, 200, 140, 1)'
        ctx.shadowColor = 'rgba(220, 195, 100, 0.5)'
        ctx.shadowBlur = 2
        ctx.beginPath()
        ctx.arc(p.x, p.y, p.r, 0, Math.PI * 2)
        ctx.fill()
        ctx.restore()
      }
    }

    function drawLightRays(t: number, alpha: number) {
      for (let i = 0; i < 3; i++) {
        const angle = (15 + i * 20) * Math.PI / 180
        const ox    = W * (0.22 + i * 0.28)
        const len   = H * 1.3
        const w2    = 70 + i * 45
        const op    = (0.055 + i * 0.01) * (0.65 + 0.35 * Math.sin(t * 0.12 + i)) * alpha
        ctx.save()
        ctx.filter = 'blur(18px)'
        ctx.translate(ox, -H * 0.08)
        ctx.rotate(angle)
        const g2 = ctx.createLinearGradient(0, 0, 0, len)
        g2.addColorStop(0,    `rgba(190, 225, 145, 0)`)
        g2.addColorStop(0.04, `rgba(190, 225, 145, ${op})`)
        g2.addColorStop(0.55, `rgba(180, 215, 130, ${op * 0.35})`)
        g2.addColorStop(1,    'rgba(0,0,0,0)')
        ctx.fillStyle = g2
        ctx.beginPath()
        ctx.moveTo(-8, 0); ctx.lineTo(8, 0)
        ctx.lineTo(w2, len); ctx.lineTo(-w2, len)
        ctx.closePath(); ctx.fill()
        ctx.restore()
      }
    }

    function drawForestEdge(t: number, alpha: number) {
      if (!forestTrees.length) return
      const wind = 0.48 + 0.44 * Math.sin(t * 0.13) + 0.08 * Math.sin(t * 0.44 + 0.7)
      const breathe = 0.90 + 0.10 * Math.sin(t * 0.07)
      const hg = ctx.createLinearGradient(0, H * 0.64, 0, H * 0.80)
      hg.addColorStop(0,    'rgba(0,0,0,0)')
      hg.addColorStop(0.38, `rgba(20, 62, 28, ${0.72 * alpha * breathe})`)
      hg.addColorStop(1,    'rgba(0,0,0,0)')
      ctx.fillStyle = hg
      ctx.fillRect(0, H * 0.64, W, H * 0.16)

      for (const layer of forestTrees) {
        ctx.save()
        ctx.globalAlpha = alpha * layer.op
        ctx.fillStyle   = layer.color
        ctx.beginPath()
        const ts = layer.trees
        ctx.moveTo(-12, H + 4)
        ctx.lineTo(-12, ts[0].baseY)
        for (const tr of ts) {
          const sway = (Math.sin(t * 0.48 + tr.phase)  * 0.07
                      + Math.sin(t * 1.10 + tr.phase2) * 0.025) * tr.hw * 2 * wind
          const px  = tr.cx + sway
          const py  = tr.baseY - tr.h
          const lx  = tr.cx - tr.hw + sway
          const rx  = tr.cx + tr.hw + sway
          const cpY = py + tr.h * 0.14
          ctx.lineTo(lx, tr.baseY)
          ctx.quadraticCurveTo(px - tr.hw * 0.20, cpY, px, py)
          ctx.quadraticCurveTo(px + tr.hw * 0.20, cpY, rx, tr.baseY)
        }
        ctx.lineTo(W + 12, ts[ts.length - 1].baseY)
        ctx.lineTo(W + 12, H + 4)
        ctx.closePath(); ctx.fill()
        ctx.restore()
      }

      const fogH = H * 0.16
      const fogG = ctx.createLinearGradient(0, H - fogH, 0, H)
      fogG.addColorStop(0,   'rgba(0,0,0,0)')
      fogG.addColorStop(0.5, `rgba(3,12,5,${0.20 * alpha})`)
      fogG.addColorStop(1,   `rgba(2,8,4,${0.38 * alpha})`)
      ctx.fillStyle = fogG
      ctx.fillRect(0, H - fogH, W, fogH)
    }

    function drawForestBirds(dt: number, t: number, alpha: number) {
      const ms = 0.32
      for (let i = 0; i < birds.length; i++) {
        const b = birds[i]
        b.x += b.vx * ms * dt
        b.y += (b.vy + Math.sin(t * 0.18 + b.phase) * 6) * ms * dt
        if (b.x < -b.hw - 6) b.x = W + b.hw + 6
        if (b.x > W + b.hw + 6) b.x = -b.hw - 6
        if (b.y < H * 0.08) b.vy =  Math.abs(b.vy) + 1
        if (b.y > H * 0.64) b.vy = -(Math.abs(b.vy) + 1)
        const flap = Math.sin(t * b.flapRate + b.phase) * b.flapAmp
        const bow  = b.hw * 0.48
        const tipY = b.y + flap * 0.55
        ctx.save()
        ctx.globalAlpha = b.op * alpha
        ctx.strokeStyle = 'rgba(220, 245, 225, 1)'
        ctx.lineWidth   = 2.2
        ctx.lineCap     = 'round'
        ctx.lineJoin    = 'round'
        ctx.beginPath()
        ctx.moveTo(b.x - b.hw, tipY)
        ctx.quadraticCurveTo(b.x - b.hw * 0.28, b.y - bow, b.x, b.y)
        ctx.quadraticCurveTo(b.x + b.hw * 0.28, b.y - bow, b.x + b.hw, tipY)
        ctx.stroke()
        ctx.restore()
      }
    }

    function drawDrift(dt: number, t: number, scene: Scene, alpha: number) {
      const ms = scene.motionSpeed
      for (let i = 0; i < drift.length; i++) {
        const p = drift[i]
        p.x += (p.vx + Math.cos(t * 0.3 + p.phase) * p.waveAmp * 0.5) * ms * dt
        p.y += (p.vy + Math.sin(t * 0.25 + p.phase) * 8) * ms * dt
        if (p.x < -20) p.x = W + 10
        if (p.x > W + 20) p.x = -10
        if (p.y < -20) p.y = H + 10
        if (p.y > H + 20) p.y = -10
        const twinkle = 0.60 + 0.40 * Math.sin(t * 0.8 + p.phase)
        ctx.save()
        ctx.globalAlpha = p.op * twinkle * alpha
        ctx.fillStyle = 'rgba(80, 190, 200, 1)'
        ctx.shadowColor = 'rgba(60, 180, 200, 0.4)'
        ctx.shadowBlur = 3
        ctx.beginPath(); ctx.arc(p.x, p.y, p.r, 0, Math.PI * 2); ctx.fill()
        ctx.restore()
      }
    }

    function drawCaustics(t: number, alpha: number) {
      for (let i = 0; i < 5; i++) {
        const cx2 = W * (0.1 + i * 0.2) + Math.sin(t * 0.22 + i) * 80
        const cy2 = H * (0.55 + Math.cos(t * 0.18 + i * 0.7) * 0.25)
        const r2  = 80 + Math.sin(t * 0.3 + i) * 40
        const op  = (0.022 + 0.012 * Math.sin(t * 0.55 + i)) * alpha
        const g2  = ctx.createRadialGradient(cx2, cy2, 0, cx2, cy2, r2)
        g2.addColorStop(0, `rgba(60, 180, 220, ${op})`)
        g2.addColorStop(1, 'rgba(0,0,0,0)')
        ctx.fillStyle = g2
        ctx.beginPath(); ctx.arc(cx2, cy2, r2, 0, Math.PI * 2); ctx.fill()
      }
    }

    function drawEmbers(dt: number, t: number, scene: Scene, alpha: number) {
      const ms = scene.motionSpeed
      for (let i = 0; i < embers.length; i++) {
        const p = embers[i]
        p.x += (p.vx + Math.sin(t * 1.2 + p.phase) * 18) * ms * dt
        p.y += p.vy * ms * dt
        p.op -= dt * 0.06
        if (p.y < -10 || p.op <= 0) { embers[i] = makeEmber(W, H, false); continue }
        const flick = 0.5 + 0.5 * Math.sin(t * 8 + p.phase)
        ctx.save()
        ctx.globalAlpha = Math.max(0, p.op) * flick * alpha
        ctx.fillStyle = `rgba(${220 + Math.random() * 35 | 0}, ${60 + Math.random() * 60 | 0}, 15, 1)`
        ctx.shadowColor = 'rgba(255, 120, 30, 0.7)'; ctx.shadowBlur = 6
        ctx.beginPath(); ctx.arc(p.x, p.y, p.r, 0, Math.PI * 2); ctx.fill()
        ctx.restore()
      }
    }

    function drawFireFlicker(t: number, alpha: number) {
      const fl = 0.04 * (0.5 + 0.5 * Math.sin(t * 7.3)) +
                 0.03 * (0.5 + 0.5 * Math.sin(t * 11.7)) +
                 0.02 * (0.5 + 0.5 * Math.sin(t * 19.1))
      const g2 = ctx.createRadialGradient(W * 0.5, H * 0.88, 0, W * 0.5, H * 0.88, W * 0.6)
      g2.addColorStop(0,   `rgba(200, 70, 10, ${fl * alpha})`)
      g2.addColorStop(0.4, `rgba(160, 45, 5, ${fl * 0.5 * alpha})`)
      g2.addColorStop(1,   'rgba(0,0,0,0)')
      ctx.fillStyle = g2; ctx.fillRect(0, 0, W, H)
    }

    function drawStars(_dt: number, t: number, _scene: Scene, alpha: number) {
      const scrollY = window.scrollY || 0
      for (let i = 0; i < stars.length; i++) {
        const p = stars[i]
        const parallax = (i % 3) * 0.003
        const sx = p.x - scrollY * parallax
        const sy = p.y - scrollY * parallax * 0.5
        const twinkle = 0.3 + 0.7 * Math.sin(t * p.twinkleSpeed + p.twinklePhase)
        ctx.save()
        ctx.globalAlpha = p.op * twinkle * alpha
        ctx.fillStyle = 'rgba(220, 225, 255, 1)'
        if (p.r > 0.8) { ctx.shadowColor = 'rgba(200, 215, 255, 0.6)'; ctx.shadowBlur = 4 }
        ctx.beginPath(); ctx.arc(sx, sy, p.r, 0, Math.PI * 2); ctx.fill()
        ctx.restore()
      }
    }

    function drawStreaks(dt: number, _t: number, scene: Scene, alpha: number) {
      const ms = scene.motionSpeed
      for (let i = 0; i < streaks.length; i++) {
        const p = streaks[i]
        p.x += p.speed * ms * dt
        if (p.x - p.len > W + 20) { streaks[i] = makeStreak(W, H, false); continue }
        const g2 = ctx.createLinearGradient(p.x - p.len, p.y, p.x, p.y)
        g2.addColorStop(0,   'rgba(140,160,220,0)')
        g2.addColorStop(0.3, `rgba(140,160,220,${p.op * alpha})`)
        g2.addColorStop(0.7, `rgba(140,160,220,${p.op * alpha})`)
        g2.addColorStop(1,   'rgba(140,160,220,0)')
        ctx.save()
        ctx.strokeStyle = g2; ctx.lineWidth = p.w; ctx.lineCap = 'round'
        ctx.beginPath(); ctx.moveTo(p.x - p.len, p.y); ctx.lineTo(p.x, p.y); ctx.stroke()
        ctx.restore()
      }
    }

    function drawCityLights(dt: number, _t: number, alpha: number) {
      for (let i = 0; i < cityLights.length; i++) {
        const p = cityLights[i]
        p.life += dt
        if (p.state === 'in')  p.op = Math.min(p.maxOp, p.op + dt * 1.5)
        if (p.state === 'out') p.op = Math.max(0, p.op - dt * 2.0)
        if (p.op >= p.maxOp && p.state === 'in') p.state = 'hold'
        if (p.state === 'hold' && p.life > p.maxLife * 0.6) p.state = 'out'
        if (p.state === 'out' && p.op <= 0) { cityLights[i] = makeCityLight(W, H); continue }
        const [cr, cg, cb] = p.col
        ctx.save()
        ctx.globalAlpha = p.op * alpha
        ctx.fillStyle = `rgb(${cr},${cg},${cb})`
        ctx.shadowColor = `rgba(${cr},${cg},${cb},0.5)`; ctx.shadowBlur = 5
        ctx.beginPath(); ctx.arc(p.x, p.y, p.r, 0, Math.PI * 2); ctx.fill()
        ctx.restore()
      }
    }

    function drawLightning(_t: number, flashT2: number, alpha: number) {
      if (flashT2 <= 0) return
      const cx = boltPath ? boltPath.segs[0][0] : W * 0.5
      const grd = ctx.createRadialGradient(cx, H * 0.12, 0, cx, H * 0.12, W * 0.65)
      grd.addColorStop(0,    `rgba(215, 228, 255, ${flashT2 * 0.09 * alpha})`)
      grd.addColorStop(0.45, `rgba(195, 215, 255, ${flashT2 * 0.03 * alpha})`)
      grd.addColorStop(1,    'rgba(0,0,0,0)')
      ctx.fillStyle = grd; ctx.fillRect(0, 0, W, H)
    }

    function drawParticles(
      type: Scene['particles'] | undefined,
      dt: number, t: number, scene: Scene, alpha: number, update: boolean
    ) {
      if (alpha < 0.01 || !type) return
      const d = update ? dt : 0
      switch (type) {
        case 'rain':    drawRain(d, scene, alpha); break
        case 'mist':    drawMist(d, t, scene, alpha); break
        case 'pollen':  drawPollen(d, t, scene, alpha); break
        case 'drift':   drawDrift(d, t, scene, alpha); break
        case 'embers':  drawEmbers(d, t, scene, alpha); break
        case 'stars':   drawStars(d, t, scene, alpha); break
        case 'streaks': drawStreaks(d, t, scene, alpha); break
      }
    }

    let nextFlash = 4 + Math.random() * 12

    const tick = (ts: number) => {
      const dt = Math.min((ts - lastTs) / 1000, 0.05)
      lastTs = ts

      const body = document.body
      if (document.hidden ||
          body.classList.contains('breath-focus') ||
          body.classList.contains('has-journey-open') ||
          body.classList.contains('amb-sleep')) {
        ctx.clearRect(0, 0, W, H)
        animId = requestAnimationFrame(tick)
        return
      }

      const t = ts / 1000
      const fallbackScene: Scene = window.__mindspaceScene ?? {
        id: 'midnight-rain', label: 'Midnight Rain', glyph: '⋮',
        story: '', whispers: [], companion: [],
        sky: { floor: [0.026,0.030,0.082], horizon: [0.295,0.215,0.255], mid: [0.092,0.098,0.190], deep: [0.030,0.038,0.095], band: [0.200,0.140,0.220], stars: 0.20 },
        tint: [8, 14, 48], tintOpacity: 0.22,
        glow: [55, 95, 210], fog: [12, 25, 72], fogDensity: 0.55,
        particles: 'rain', lightning: true, rays: false,
        caustics: false, flicker: false, cityLights: false, motionSpeed: 0.80,
      }
      const scene     = window.__mindspaceScene ?? fallbackScene
      const prevScene = window.__mindspaceScenePrev
      const sceneT    = window.__mindspaceSceneT ?? 1
      const eT        = easeInOut(sceneT)

      ctx.clearRect(0, 0, W, H)

      if (prevScene && eT < 1) {
        drawColorGrade(prevScene, 1 - eT)
        drawColorGrade(scene, eT)
        drawSceneGlow(prevScene, 1 - eT)
        drawSceneGlow(scene, eT)
        drawFog(prevScene, t, 1 - eT)
        drawFog(scene, t, eT)
      } else {
        drawColorGrade(scene, 1)
        drawSceneGlow(scene, 1)
        drawFog(scene, t, 1)
      }

      const pAlpha2    = eT
      const pAlphaPrev = prevScene ? 1 - eT : 0

      if (prevScene && pAlphaPrev > 0.01) {
        drawParticles(prevScene.particles, dt, t, prevScene, pAlphaPrev, false)
      }
      drawParticles(scene.particles, dt, t, scene, pAlpha2 > 0.01 ? pAlpha2 : 1, true)

      if (scene.caustics)               drawCaustics(t, eT)
      if (scene.flicker)                drawFireFlicker(t, eT)
      if (scene.cityLights)             drawCityLights(dt, t, eT)
      if (scene.rays)                   drawLightRays(t, eT)
      if (scene.id === 'forest-temple') {
        const fa = eT > 0.01 ? eT : 1
        drawForestEdge(t, fa)
        drawForestBirds(dt, t, fa)
      }

      if (scene.lightning) {
        nextFlash -= dt
        if (nextFlash <= 0) {
          flashT    = 1
          boltPath  = generateBolt()
          nextFlash = 6 + Math.random() * 18
          if (window.MindSpaceTriggerLightning) window.MindSpaceTriggerLightning()
        }
      }
      if (flashT > 0) {
        flashT = Math.max(0, flashT - dt * 6)
        drawLightning(t, flashT, eT)
      }

      animId = requestAnimationFrame(tick)
    }

    window.addEventListener('resize', resize)
    resize()
    animId = requestAnimationFrame(tick)

    return () => {
      cancelAnimationFrame(animId)
      window.removeEventListener('resize', resize)
    }
  }, []) // eslint-disable-line react-hooks/exhaustive-deps

  if (REDUCED_ATM) return null
  return <canvas ref={canvasRef} className="atm-canvas" aria-hidden="true" />
}

/* ═══════════════════════════════════════════════════════════════
   Scene Switcher UI
═══════════════════════════════════════════════════════════════ */
export function SceneSwitcher() {
  const allScenes = SCENES
  const ids       = sceneEngine.getIds()

  const [open,   setOpen]   = useState(false)
  const [active, setActive] = useState('midnight-rain')
  const [pulse,  setPulse]  = useState(false)

  useEffect(() => {
    const off = sceneEngine.onChange(s => setActive(s.id))
    const seen = localStorage.getItem('mindspace.scenePulsed')
    if (!seen) {
      const t1 = setTimeout(() => {
        setPulse(true)
        const t2 = setTimeout(() => {
          setPulse(false)
          localStorage.setItem('mindspace.scenePulsed', '1')
        }, 5200)
        return () => clearTimeout(t2)
      }, 3200)
      return () => { off(); clearTimeout(t1) }
    }
    return off
  }, []) // eslint-disable-line react-hooks/exhaustive-deps

  const pick = (id: string) => {
    sceneEngine.setScene(id)
    setActive(id)
    setOpen(false)
    setPulse(false)
  }

  if (ids.length === 0) return null
  const cur = allScenes[active] ?? allScenes[ids[0]]

  return (
    <div className={`scene-sw${open ? ' open' : ''}`}>
      {open && (
        <div className="scene-panel">
          <div className="scene-panel-label">atmosphere</div>
          {ids.map(id => {
            const s = allScenes[id]
            return (
              <button
                key={id}
                className={`scene-opt${active === id ? ' on' : ''}`}
                onClick={() => pick(id)}
              >
                <span className="scene-opt-glyph">{s.glyph}</span>
                <span className="scene-opt-body">
                  <span className="scene-opt-name">{s.label}</span>
                  {s.story && <span className="scene-opt-story">{s.story}</span>}
                </span>
                {active === id && <span className="scene-opt-tick">·</span>}
              </button>
            )
          })}
        </div>
      )}
      <button
        className={`scene-toggle${pulse ? ' pulse' : ''}`}
        onClick={() => setOpen(o => !o)}
        aria-label="Switch scene"
        title="Change atmosphere"
      >
        <span className="scene-toggle-glyph">{cur?.glyph ?? '✦'}</span>
        <span className="scene-toggle-label">{cur?.label ?? 'Scene'}</span>
      </button>
    </div>
  )
}
